...
def to_ztf_lc(time, mag, magerr=None, time_col_name="mjd", time_col_format="mjd"):
    ...
    lc = Table(
        [time * u.day, mag * u.mag, magerr * u.mag],
        names=(f"{time_col_name}", "mag", "magerr"),
    )  # pylint: disable=no-member
    lc.add_column(
        Time(lc[f"{time_col_name}"], format=f"{time_col_format}"), name="time", index=0
    )
...
def load_ztf_lc(
    ...
    sort=True,
    verbose=False,
    add_tess_time_column=True,
    use_tess_time_as_index=False,
    time_format="mjd",
    time_scale="tdb",
):
    """ """
    if source is not None:
        if (ra is not None) or (dec is not None):
            print(
                f"Please only supply either the ra, dec arguments OR the source argument, but not both."
            )
            return
    ...
    lc.add_column(
        Time(lc[f"{time_column}"], format=f"{time_format}", scale=f"{time_scale}"),
        name="time",
        index=0,
    )
    lc = TimeSeries(data=lc)
...
def preprocess_ztf_lc(
    lc, remove_outliers=True, sigma=3, remove_trend=True, degree=1, time_column="mjd"
):
    """
...
def bin_lc(
    ...
    binned_lc = aggregate_downsample(
        lc_copy, time_bin_size=time_bin_size, aggregate_func=aggregate_func
    )
    return binned_lc
